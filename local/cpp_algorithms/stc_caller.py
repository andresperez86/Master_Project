...
def stc_caller(matrix):
    ...
    r = int(bound[0] / 2)
    c = int(bound[1] / 2)
    grid = [[((max(abs((r - 1) / 2 - i), abs((c - 1) / 2 - j)) + 1) if (
        all(pt for pg in matrix[2 * i:2 * (i + 1), 2 * j:2 * (j + 1)] for pt in pg)) else -1) for j in range(c)] for i
            in range(r)]
    vertices = {}
    obstacles = {}
    for i in range(r):
    ...
    while len(vertices):
        ...
        if grt > 0:
            ind = tmp.index(grt)
            s = (s[0] - 1, s[1]) if ind == 0 else (s[0], s[1] + 1) if ind == 1 else (s[0] + 1, s[1]) if ind == 2 else (
            s[0], s[1] - 1)
        else:
            pathf.append(pathi)
            pathi = {}
    ...
    for p in range(len(pathf)):
        ...
        for i in range(1, len(path)):
            ind = 0 if path[i - 1][0] - 1 == path[i][0] else 1 if path[i - 1][1] + 1 == path[i][1] else 2 if \
            path[i - 1][0] + 1 == path[i][0] else 3
    ...
    for p in range(len(ejPath)):
        path = ejPath[p]
        ind = 0 if path[0][0] - 1 == path[1][0] else 1 if path[0][1] + 1 == path[1][1] else 2 if path[0][0] + 1 == \
                                                                                                 path[1][0] else 3
        treef[path[0]][ind] = 1
